...
//     for (int x1 = lo_x; x1 < hi_x; x1 += 1) {
//         for (int y1 = lo_y; y1 < hi_y; y1 += 1) {
//             Interval x_box = pix.x.meet(Interval(x1, x1 + 1));
//             Interval y_box = pix.y.meet(Interval(y1, y1 + 1));
...
//             double alpha = img.find_pixel(x1, y1, pix.channel).inf;
//             double beta = img.find_pixel(x1, y1 + 1, pix.channel).inf;
//             double gamma = img.find_pixel(x1 + 1, y1, pix.channel).inf;
//             double delta = img.find_pixel(x1 + 1, y1 + 1, pix.channel).inf;

//             // Translation 1d
//              double min_delta = max(x1 - cx, y1 - cy);
//              double max_delta = min(x1 + 1 - cx, y1 + 1 - cy);
//              Interval sub_delta = Interval(min_delta, max_delta).meet(it_delta);
//              DeepPoly1d px = {1, cx, 1, cx, sub_delta};
//              DeepPoly1d py = {1, cy, 1, cy, sub_delta};
...
//             Interval tmp = (alpha * (x1 + 1 - pix.x) * (y1 + 1 - pix.y)
//                                    + beta * (x1 + 1 - pix.x) * (pix.y - y1)
//                                    + gamma * (pix.x - x1) * (y1 + 1 - pix.y)
//                                    + delta * (pix.x - x1) * (pix.y - y1));

//             DeepPoly1d tmp_dp = (alpha * (-px + (x1 + 1)) * (-py + (y1 + 1))
//                                         + beta * (-px + (x1 + 1)) * (py - y1)
//                                         + gamma * (px - x1) * (-py + (y1 + 1))
//                                         + delta * (px - x1) * (py - y1));
...
//             if (lower) {
//                 alpha = img.find_pixel(x1, y1, pix.channel).inf;
//                 beta = img.find_pixel(x1, y1 + 1, pix.channel).inf;
//                 gamma = img.find_pixel(x1 + 1, y1, pix.channel).inf;
//                 delta = img.find_pixel(x1 + 1, y1 + 1, pix.channel).inf;
//             } else {
//                 alpha = img.find_pixel(x1, y1, pix.channel).sup;
//                 beta = img.find_pixel(x1, y1 + 1, pix.channel).sup;
//                 gamma = img.find_pixel(x1 + 1, y1, pix.channel).sup;
//                 delta = img.find_pixel(x1 + 1, y1 + 1, pix.channel).sup;
//             }
...
//             ++cnt;
//             dfdx = dfdx.join(((gamma - alpha) * (y1 + 1 - pix.y) + (delta - beta) * (pix.y - y1)));
//             dfdy = dfdy.join(((beta - alpha) * (x1 + 1 - pix.x) + (delta - gamma) * (pix.x - x1)));
//         }
//     }
